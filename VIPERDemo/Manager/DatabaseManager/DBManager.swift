...
class DBManager : NSObject {
    ...
    let coredataQueue = DispatchQueue(label: "com.cybage.gofigure.coredata", attributes: [])
    
    /**
     The directory the application uses to store the Core Data store file. This code uses a directory named "CYB.ContactsApp" in the application's documents Application Support directory.
     */
    ...
    lazy private var managedObjectModel: NSManagedObjectModel = {
        let modelURL = Bundle.main.url(forResource: "VIPERDemo", withExtension: "momd")!
    ...
    lazy private var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        logFor.DLog(message:"documents url == \(applicationDocumentsDirectory)")
            ...
            do {
                ...
                self.parentManagedObjectContext.perform {
                    do {
                        ...
                        self.logFor.DLog(message:"error in top most backgrond thread MOC ")
    ...
    /// It delete the managed object (entity).
    ///
    /// - Parameters:
    ...
    /// It pull the managed object (entity) for the given Ids.
    ///
    /// - Parameters:
    ///   - objectIDs: ManagedObjectID's.
    ///   - backgrounContext: Managed Object Context.
    ///   - completion: Callback.
    func fetchEntity(withManagedObjectIDs objectIDs : [NSManagedObjectID] , managedObjectContext backgrounContext : NSManagedObjectContext, completion:@escaping ([NSManagedObject]?) -> Void) {

        var managedObjects = [NSManagedObject]()
        backgrounContext.perform {
            for managedObjectID in objectIDs {
                let record = backgrounContext.object(with: managedObjectID)
                managedObjects.append(record)
            }
            completion(managedObjects)
        }
    }
    
    /// It pull the managed object (entity) for the given Id.
    ///
    /// - Parameters:
    ...
    func fetchEntity(_ entityName : String , sortDescriptors : [NSSortDescriptor]? , predicate : NSPredicate?, managedObjectContext backgrounContext : NSManagedObjectContext, completion:@escaping (NSManagedObject?) -> Void) {
        ...
        backgrounContext.perform {
            ...
            do {
                ...
                self.logFor.DLog(message:"error in fetching records from database")
            }
            completion(record)
        }
    }
    ...
    func entityCount(fromEntity entityName : String , sortDescriptors : [NSSortDescriptor]? , predicate : NSPredicate?, managedObjectContext backgrounContext : NSManagedObjectContext) -> Int{
        ...
        do {
            ...
            self.logFor.DLog(message:"failed to catch")
        }
        return 0
    }
